...
// Copy output values from last round to outData 	
     ...
     * ****************************** Key Schedule *******************************
     */
    private static void SeedRoundKey(byte pbUserKey[]) // [in]	secret user key
    {
        int A[] = new int[1];
        int B[] = new int[1];
        int C[] = new int[1];
...
// reorder for little endian	  
    ...
    private static void SetKey(String key) throws Exception {
        byte pbUserKey[] = SecureHash.getHash("MD5", key.getBytes());
        SeedRoundKey(pbUserKey);
    }

    private static void SetKey(byte[] key) throws Exception {
        SeedRoundKey(key);
    }

    /**
     * SEED 암호화 기본형
     * @param plainText 평문 바이트 배열
     * @param pwd 대칭키 암호
     * @return 암호화된 데이터 바이트 배열
     * @throws Exception 
     */
    public static byte[] encrypt(byte[] plainText, String pwd) throws Exception{
        //16 bit Padding
        int max = 0;
        if (plainText.length % pad != 0) {
            max = pad - (plainText.length % pad);
        }

        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        baos.write(plainText);
        byte[] nb = new byte[max];
        ...
        byte pbData[] = baos.toByteArray();

        int size = pbData.length;

        byte pbCipher[] = new byte[pad];
        byte reData[][] = new byte[size / pad][pad];
    ...
    /**
     * SEED 암호화 문자열에 대한 Char Set을 지정하여 plainText.getBytes(encoding)
     * 으로 변한한 후에 SEED 암호화 기본 함수 호출
     ...
     * @throws Exception 
     */
    public static byte[] encrypt(String plainText, String pwd, String charset) throws Exception {
        return encrypt(plainText.getBytes(charset), pwd);
    }
    
    /**
     * 평문 문자열을 plainText.getBytes()로 변환하여
     * SEED 암호화 기본 함수 호출
     * @param plainText 평문 문자열
     * @param pwd 대칭키 암호
     * @return 암호화 데이터 바이브 배열
     * @throws Exception 
     */
    public static byte[] encrypt(String plainText, String pwd) throws Exception {
        return encrypt(plainText.getBytes(), pwd);
    }

    /**
     * SEED 복호화 기본 함수
     * @param cipher 암호화된 데이터 바이트 배열
     * @param pwd 대칭키 암호
     * @return 복호화된 데이터 바이트 배열 - 문자열인 경우 new String()을 사용하여 문자열로 변환
     * @throws Exception 
     */
    public static byte[] decrypt(byte[] cipher, String pwd) throws Exception {
        byte[] rePlain= null;

        int size = cipher.length;
        byte pbPlain[] = new byte[pad];
        byte reData[][] = new byte[size / pad][pad];
        rePlain = new byte[size];
        ...
        for (int i = 0; i < size; i++) {
            b1 = (i / pad);
            b2 = (i % pad);
            reData[b1][b2] = cipher[i];
        }
        int rf = 0;
        for (int i = 0; i < reData.length; i++) {
            SeedDecrypt(reData[i], pbPlain);
    ...
    /**
     * 대칭키 내장 encryptPiggyBank 기본 함수
     * 입력 데이터를 압축한 후에 SEED를 이용하여 함호화한다.
     * 암호화에 사용될 대칭키는 평문 데이터의 MD5 해시값을 사용하며
     * 암호화된 문자열에 포함된다.
     * @param plainText 평문 데이터 바이트 배열
     * @return Base64로 인코딩된 문자열
     * @throws Exception 
     */
    public static String encryptPiggyBank(byte[] plainText) {
        String ret = "";
        try {
            //String pwd = SecureHash.getHashHex("MD5", plainText);
            SecureRandom random = new SecureRandom();
            String pwd = SecureHash.getHashHex("MD5", String.valueOf(random.nextLong()).getBytes());
            byte[] cipher = encrypt(plainText, pwd);
            String scipher = new String(Base64Utils.encode(cipher, false));

            ret = SecureKey.EmbedKey(scipher, pwd);
        }catch (Exception e){
    ...
    /**
     * 입력 문자열을 plainText.getBytes(charset)로 변환한 후 대칭키 내장 encryptPiggyBank 기본 함수를 호출한다.
     * @param plainText 평문 문자열
     * @param charset Char Set
     * @return Base64로 인코딩된 문자열
     * @throws Exception 
     */
    public static String encryptPiggyBank(String plainText, String charset) throws Exception{
        return encryptPiggyBank(plainText.getBytes(charset));
    }
    
    /**
     * encryptPiggyBank 기본 함수
     * 입력 데이터를 압축한 후에 SEED를 이용하여 함호화한다.
     * @param plainText 평문 데이터 바이트 배열
     * @param pwd 대칭키
     * @return Base64로 인코딩된 문자열
     * @throws Exception 
     */
    public static String encryptPiggyBank(byte[] plainText, String pwd) throws Exception{
        byte[] cipher = encrypt(plainText, pwd);
        return new String(Base64Utils.encode(cipher, false));
    }
    
    /**
     * 입력 문자열을 plainText.getBytes(charset)로 변환한 후 encryptPiggyBank 기본 함수를 호출한다.
     * @param plainText 평문 문자열
     * @param pwd 대칭키
     * @param charset Char Set
     * @return Base64로 인코딩된 문자열
     * @throws Exception 
     */
    public static String encryptPiggyBank(String plainText, String pwd, String charset) throws Exception{
        return encryptPiggyBank(plainText.getBytes(charset), pwd);
    }
    
    /**
     * 대칭키 내장 decryptPiggyBank 기본 함수
     * @param cipher 암호화된 문자열
     * @return 평문 바이트 배열 - 평문이 문자열인 경우 new String()을 이용하여 문자열로 변환한다.
     * @throws Exception 
     */
    public static byte[] decryptPiggyBank(String cipher) {
        byte[] ret = null;
        try {
            String[] sdata = SecureKey.ExtractKey(cipher, 32);
            byte[] b = Base64Utils.decode(sdata[0].getBytes());
            ret = decrypt(b, sdata[1]);
        }catch (Exception e){
            Log.e("Crypt", e.getMessage());
            FirebaseCrash.log(StackTrace.getStackTrace(e));
            FirebaseCrash.report(e);
    ...
    /**
     * 대칭키 내장 문자열 반환 decryptPiggyBank 함수
     * @param cipher 암호화된 문자열
     * @param charset 복호화된 결과의 Char Set
     * @return 복호화된 문자열
     * @throws Exception 
     */
    public static String decryptPiggyBank(String cipher, String charset) throws Exception{
        byte[] b = decryptPiggyBank(cipher);
        return new String(b, charset);
    }
    
    /**
     * 문자열 반환 decryptPiggyBank 함수
     * @param cipher 암호화된 문자열
     * @param pwd 대칭키
     * @param charset 복호화된 결과의 Char Set
     * @return 복호화된 문자열
     * @throws Exception 
     */
    public static String decryptPiggyBank(String cipher, String pwd, String charset) throws Exception{
        byte[] b = Base64Utils.decode(cipher.getBytes());
        byte[] c = decrypt(b, pwd);
    ...
    public static void main(String[] args)  {
        try{
            ...
            String ss = Crypt.encryptPiggyBank(s.getBytes());
            System.out.println(ss);
            String s0 = new String(Crypt.decryptPiggyBank(ss));

...
public class HyundaiCardParser implements IParser, Serializable {
    private static final long serialVersionUID = SmsUtils.superSerialVersionUID;

    private static final String TAG = "HyundaiCardParser";
    ...
    public void parse(ArrayList<String> body) {
        try {
            ...
            if (SmsUtils.getResource(R.string.send_web).equals(web)) {
                cardname = CardEnum.HYUNDAE_CARD;
                if (body.size() > 3 && isNum(body.get(3))) {
                    accept = true;
                    amount = body.get(3);
                    ussage = body.get(5);
